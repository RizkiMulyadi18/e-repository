            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-[#FAF7EE] border-b-2 border-black text-[11px] font-black uppercase text-black tracking-wider">
                        ...
                        <th class="py-3.5 px-4">Jumlah Dokumen</th>
                ...
                <tbody class="divide-y-2 divide-neutral-100 text-xs font-bold text-neutral-800">
                    @forelse ($categories as $cat)
                        <tr class="hover:bg-[#FFFDF7] transition-colors">
                            <td class="py-4 px-6">
                                <span class="px-3 py-1 bg-white border-2 border-black rounded-xl text-xs font-black shadow-[2px_2px_0px_#000] inline-block">
                                    {{ $cat->name }}
                                </span>
                            </td>
                            <td class="py-4 px-4 font-mono text-[11px] text-neutral-500">
                                {{ $cat->slug }}
                            </td>
                            <td class="py-4 px-4">
                                <span class="px-2.5 py-0.5 bg-mint/30 border border-black rounded-md text-[11px] font-black">
                                    {{ $cat->dokumens_count ?? 0 }} Dokumen
                                </span>
                            </td>
                            ...
                            <td class="py-4 px-6 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button wire:click="openEditModal({{ $cat->id }})" title="Edit Kategori"
                                    ...
                                    <button wire:click="confirmDelete({{ $cat->id }})" title="Hapus Kategori"
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div class="bg-white border-4 border-black rounded-3xl shadow-brutal-xl w-full max-w-md overflow-hidden animate-fade-in-up">
                ...
                <form wire:submit="save" class="p-6 space-y-4">
                    <div class="space-y-1">
                        <label class="block text-xs font-black uppercase text-black">Nama Kategori *</label>
                        <input wire:model.live.debounce.300ms="name" type="text" required placeholder="Contoh: Skripsi Informatika"
                    ...
                    <div class="space-y-1">
                        ...
                        <input wire:model="slug" type="text" required placeholder="skripsi-informatika"
    ...
    @if ($isDeleteModalOpen)
        <div class="fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div class="bg-white border-4 border-black rounded-3xl shadow-brutal-xl w-full max-w-md p-6 text-center space-y-4">
                ...
                <div class="space-y-1">
                    <h3 class="text-lg font-black text-black">Hapus Kategori?</h3>
                    <p class="text-xs font-semibold text-neutral-600">
                        Kategori ini akan dihapus dari sistem.
                    </p>
                </div>
